...
const Certificate = ({ currentCourse, currentUser, navTo, currentCertificate }) => {
  const certificateData = useMemo(
    () => ({
      studentName: currentUser?.name || 'Nome do Colaborador',
      courseName: currentCourse?.name || 'Nome do Curso',
      hours:
        (currentCourse?.estimated_duration_minutes / 60)?.toFixed(1) || '--',
      date: new Date().toLocaleDateString('pt-BR'),
      code: currentCertificate?.signature ||
        crypto.randomUUID().toUpperCase().substring(0, 18),
    }),
    [currentCourse, currentUser, currentCertificate]
  );

  return (
    <div
      id="certificate-view"
      className="view p-0 certificate-page"
    >
      {/* Botão volta – só na tela, não na impressão */}
      <button
      ...
      <div className="certificate-border shadow-2xl">
        <div className="certificate-inner-border">
          <div className="mb-8">
            <h1 className="text-4xl font-extrabold text-blue-700 tracking-widest uppercase">
              CERTIFICADO
            </h1>
            <p className="text-blue-500 uppercase tracking-wider text-sm mt-1">
              de Conclusão
            </p>
          </div>
          ...
          <div className="my-8">
            ...
            <div className="max-w-xl mx-auto border-t border-b border-gray-200 py-4 my-8">
              <p className="text-gray-700 text-lg">
                Carga Horária:{' '}
                <span className="font-bold">{certificateData.hours}</span> horas{' '}
                <br />
                Data de Conclusão:{' '}
          ...
          {/* Rodapé com assinatura literal */}
          <div className="flex justify-between items-end mt-16 px-10">
            {/* BLOCO ESQUERDA – Assinatura */}
            <div className="text-center flex flex-col items-center">
              {/* Imagem da assinatura – coloque o PNG em public/assinaturas/assinatura-julio.png */}
              <img
                src="../assinaturas/assinatura-julio.png"
            ...
            {/* BLOCO DIREITA – Código / Assinatura digital */}
            <div className="text-right">
              ...
              {currentCertificate?.verify_url && (
                <p className="text-[10px] text-gray-400 mt-1">
                  Verificar em: {currentCertificate.verify_url}
                </p>
              )}
            </div>
          </div>
